...
| Whirlpool (decoded from the engine's slider module): each media card's traversal is normalized to signed progress D ∈ [−1, +1] (0 at the focus band); the card's rotation tracks D, it drifts sideways through mid-traversal on a cos/sin arc (the engine also yaws its WebGL plane 4×D and feeds D to a shader twist) — position-driven, so it works at any scroll speed | One scrubbed proxy tween per card whose `onUpdate` derives every property from the same orbit angle θ (270°→90°): `x = driftX·cosθ (+ exitX·D^1.3 past focus)`, `y = −travelY·sinθ`, rotation/yaw/shear linear in D, depth scale + zIndex, and a visibility-window alpha. The animated element is a transform-clean flex shell (layout centers it; GSAP owns all transforms) | `WhirlpoolCarousel` |
| Title swirl: titles tilt transiently with fast scrolling — more the further from the focus band, opposite signs above vs. below — and settle level at rest | Per-title `gsap.quickTo(el, 'rotation')` re-targeted from Lenis `scroll` events: `(velocity / maxVelocity) × maxTilt × centerOffset × 2` | `WhirlpoolCarousel` |
